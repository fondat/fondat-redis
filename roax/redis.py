...
class RedisResource(Resource):
    def __init__(
        ...
        self.id_schema = (
            id_schema
            or getattr(self, "id_schema", None)
            or self.schema.properties["id"]
        )
        self.ttl = ttl
    ...
    def create(self, id, _body):
        ...
        if not self.redis.set(
            self._encode_id(id), self._encode_body(_body), nx=True, px=px
        ):
    ...
    def update(self, id, _body):
        ...
        if ttl == -1:
            ttl = None
        if (ttl and ttl <= 0) or not self.redis.set(
            str_id, self._encode_body(_body), ex=ttl, xx=True
        ):
            raise NotFound(f"{self.name} item not found")
